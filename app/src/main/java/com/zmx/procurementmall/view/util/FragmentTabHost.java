...
/**
 * Special TabHost that allows the use of {@link Fragment} objects for its ab
...
public class FragmentTabHost extends TabHost implements
    ...
    private void ensureContent() {
        if (mRealTabContent == null) {
            ...
            if (mRealTabContent == null) {
                throw new IllegalStateException(
                        "No ab content FrameLayout found for id "
                                + mContainerId);
            }
        }
    }
    ...
    public void addTab(TabSpec tabSpec, Class<?> clss, Bundle args) {
        ...
        if (mAttached) {
            ...
            // sure this 's fragment is inactive if it exists. This shouldn't
            // normally happen.
...
//			if (ab.fragment != null && !ab.fragment.isDetached()) {
            if (tab.fragment != null) {
                if (tab.tag.equals(currentTab)) {
                    // The fragment for this ab is already there and
                    // active, and it is what we really want to have
                    // as the current ab. Nothing to do.
                    mLastTab = tab;
                } else {
                    // This fragment was restored in the active state,
                    // but is not the current ab. Deactivate it.
                    if (ft == null) {
                        ft = mFragmentManager.beginTransaction();
                    }
//					ft.detach(ab.fragment);
    ...
    private FragmentTransaction doTabChanged(String tabId,
        ...
        if (newTab == null) {
            throw new IllegalStateException("No ab known for tag " + tabId);
        }
        if (mLastTab != newTab) {
            if (ft == null) {
                ft = mFragmentManager.beginTransaction();
